...
class RetargetingConfig:
    ...
    def build(self) -> SeqRetargeting:
        ...
        # )
        robot_urdf = urdf.URDF.load(
            self.urdf_path, add_dummy_free_joints=False, build_scene_graph=False
        )
        urdf_name = self.urdf_path.split(os.path.sep)[-1]
        temp_dir = tempfile.mkdtemp(prefix="dex_retargeting-")
        temp_path = f"{temp_dir}/{urdf_name}"
        # temp_path = "/tmp/dex_retargeting-cuwxr13j/inspire_hand_left.urdf"
        robot_urdf.write_xml_file(temp_path)

        # Load pinocchio model
        print(f"Loading URDF from: {temp_path}")
    ...
    def build_robot_wrapper(self) -> RobotWrapper:
